...
namespace {
// Mixes one round sum into the rolling checksum value.
...
struct PthreadQueueProcessor {
    // Creates pthread workers and initializes shared queues.
    explicit PthreadQueueProcessor(std::size_t worker_count)
        ...
        for (std::size_t i = 0; i < worker_count_; ++i) {
            pthread_create(&workers_[i], NULL, &PthreadQueueProcessor::worker_entry, this);
        }
    }

    // Stops workers and destroys all pthread primitives.
    ~PthreadQueueProcessor() {
    ...
    // Runs all rounds by dispatching chunk tasks to worker queue.
...
private:
    // Bridges pthread callback entry to the processor instance.
    static void* worker_entry(void* self) {
        PthreadQueueProcessor* p = static_cast<PthreadQueueProcessor*>(self);
        p->worker_loop();
        return NULL;
    }

    // Pushes one task into the bounded internal queue.
    ...
    // Pops one task from the internal queue or stops on shutdown.
    ...
    // Processes queue tasks and updates completion counters.
    void worker_loop() {
        for (;;) {
            WorkItem item;
            if (!pop(item)) {
...
// Computes checksum in pre-C++11 serial style.
unsigned long long run_pre_cpp11_serial(const std::vector<unsigned char>& data, int rounds) {
    const std::size_t n = data.size();
    const unsigned char* ptr = data.empty() ? NULL : &data[0];
    unsigned long long checksum = 0ULL;
    for (int r = 0; r < rounds; ++r) {
...
// Computes checksum using OpenMP parallel reduction.
unsigned long long run_openmp_optimized(const std::vector<unsigned char>& data, int rounds, std::size_t omp_threads) {
    const std::size_t n = data.size();
    const unsigned char* ptr = data.empty() ? NULL : &data[0];
    unsigned long long checksum = 0ULL;
...
// Benchmarks serial, pthread queue, and OpenMP backends.
int main(int argc, char** argv) {
    ...
    const unsigned long long c0 = run_pre_cpp11_serial(data, rounds);
    const long long t1 = now_us();

    PthreadQueueProcessor pthread_processor(static_cast<std::size_t>(workers));
    const long long t2 = now_us();
    const unsigned long long c1 = pthread_processor.run(data, rounds);
    const long long t3 = now_us();

    const long long t4 = now_us();
    const unsigned long long c2 = run_openmp_optimized(data, rounds, static_cast<std::size_t>(workers));
    ...
    const int match_pthread = (c0 == c1) ? 1 : 0;
    const int match_openmp = (c0 == c2) ? 1 : 0;

    std::printf("data_mb=%d rounds=%d workers=%d\n", data_mb, rounds, workers);
    std::printf("pre_cpp11_unoptimized_us=%lld checksum=%llu\n", serial_us, c0);
    std::printf("pre_cpp11_thirdparty_pthread_us=%lld checksum=%llu\n", pthread_us, c1);
    std::printf("openmp_optimized_us=%lld checksum=%llu\n", openmp_us, c2);
    std::printf("checksum_match_pthread=%d\n", match_pthread);
    std::printf("checksum_match_openmp=%d\n", match_openmp);

    if (pthread_us > 0) {
        std::printf("speedup_thirdparty_vs_pre_cpp11=%.4f\n", static_cast<double>(serial_us) / static_cast<double>(pthread_us));
    }
    if (openmp_us > 0) {
        std::printf("speedup_openmp_vs_pre_cpp11=%.4f\n", static_cast<double>(serial_us) / static_cast<double>(openmp_us));
        std::printf("speedup_openmp_vs_thirdparty=%.4f\n", static_cast<double>(pthread_us) / static_cast<double>(openmp_us));
    }
